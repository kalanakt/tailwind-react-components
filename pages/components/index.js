		 ...
		 {components.map((item) => (
			<div className="mx-auto max-w-[85rem] px-4 sm:px-6 lg:px-8 " key={item.id}>
				<div id="scrollspy" className="divide-y divide-gray-200 dark:divide-gray-800">
					...
					{item.subcategory.map((component, index) => (
							<Link
							href="/components/[name]/[id]"
							as={"/components/" + item.name + "/" + component.name}
							passHref={true}
							key={index}
							...
							>
							    <div>
									<div className="relative pt-[50%]">
										<Image priority={true} layout='fill' loader={myLoader} className="absolute top-0 left-0 h-full w-full rounded-t-xl object-cover dark:hidden" src={component.img} alt="Image Description" />
										<Image priority={true} layout='fill' loader={myLoader} className="absolute top-0 left-0 hidden h-full w-full rounded-t-xl object-cover dark:block" src={component.img} alt="Image Description" />
									</div>
									<div className="rounded-b-xl bg-white py-3.5 px-4 dark:bg-slate-900">
										...
										<p className="mt-0.5 text-sm text-gray-400 dark:text-gray-500">{component.components.length} components</p>
									</div>
								</div>
							</Link>
					))}
